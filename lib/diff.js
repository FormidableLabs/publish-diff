...
var crlf = require("crlf-normalize").crlf;
...
var getFiles = function (filesPath, opts, callback) {
  opts = opts || {};
  ...
  readdir(filesPath, function (dirErr, files) {
    ...
    if (filter) {
      files = files
        // Remove base.
        .map(function (file) { return path.relative(filesPath, file); })
        // Apply filter.
        .filter(minimatch.filter(filter))
        // Add base back in.
        .map(function (file) { return path.join(filesPath, file); });
    }
    ...
    async.map(files, function (filePath, cb) {
      ...
      var obj = (results || []).reduce(function (memo, pair) {
        ...
        memo[relPath] = {
          relPath: relPath,
          fullPath: fullPath,
          contents: isBin ? buf : crlf(buf.toString("utf-8")),
          isBinary: isBin
        };
...
var diff = function (opts, obj, callback) {
  opts = opts || {};
  var oldName = opts.old.name;
  var newName = opts.new.name;
...
module.exports.diff = function (opts, callback) {
  ...
  async.auto({
    old: getFiles.bind(null, opts.old.files, opts),
    new: getFiles.bind(null, opts.new.files, opts),
